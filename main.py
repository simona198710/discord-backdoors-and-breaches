...
async def generate_cards_embed(channel,game_id):
    global procedures
    message = None
    
    i = 0
    while True:
        cards_list = procedures[game_id]
        embeds = []
        for j, card in enumerate(cards_list):
            embed = discord.Embed(title="Backdoors and Breaches")
            try:
                card_image = card['Image']
            except:
                card_image = "https://i.imgur.com/DrbEhX1.png"
            embed.set_image(url=card_image)
            embed.add_field(name=card['Title'], value=card['Description'])
            embed.set_footer(text=f"Card {j+1} of {len(cards_list)}")
            embeds.append(embed)
        
        if message is None:
            message = await channel.send(embed=embeds[i])
        else:
            message = await message.edit(embed=embeds[i])
        if len(embeds) > 1:
            ...
            try:
                ...
                print("Timeout error for cards embed game id",str(game_id))
                break
            else:
...
async def play_procedure(ctx, card_name: str):
    ...
    embed=discord.Embed(title="Backdoors and Breaches",description=player.mention + " Plays card:\n\n" + format_card_info(card))
    try:
        card_image = card['Image']
    except:
        card_image = "https://i.imgur.com/DrbEhX1.png"
